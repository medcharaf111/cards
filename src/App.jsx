...
const UI = {
  ...
  ar: {
    ...
    footer:"توثيق المنصة · جميع مخرجات الذكاء الاصطناعي تستلزم مراجعة بشرية قبل وصولها للطلاب",
  },
};

const ROLE_LABELS = {
  en: { Teacher:"Teacher", Student:"Student", Advisor:"Advisor", Director:"Director", Inspector:"Inspector", CNP:"CNP", AI:"AI" },
  ar: { Teacher:"معلّم", Student:"طالب", Advisor:"مستشار", Director:"مدير", Inspector:"مفتّش", CNP:"هيئة المناهج", AI:"ذكاء اصطناعي" },
};
const ROLE_META = {
...
const LessonContent = ({ lang }) => {
  ...
  const s = {
    ...
    ar: {
      ...
      p3t:"استخدم خطة من المكتبة", p3b:"تصفّح مكتبة خطط الدروس (الخزينة)، ابحث عن خطة جاهزة تناسب فصلك، واستخدمها كما هي أو أضف تعليماتك ليكيّفها الذكاء الاصطناعي لطلابك.",
...
const TimetableContent = ({ lang }) => {
  const s = {
    ...
    ar: {
      ...
      p2blue:"أزرق للمخطط،", p2green:"أخضر للمُدرَّس،", p2red:"أحمر للملغى.", p2b2:"يمكن للطلاب والمستشارين الاطلاع على التقويم.",
    },
  }[lang];
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
...
const TestGenContent = ({ lang }) => {
  ...
  const s = {
    ...
    ar: {
      intro:"تُولَّد الاختبارات تلقائياً من محتوى الدرس — لا يكتب المعلمون الأسئلة يدوياً. يراجع المعلم دائماً ويوافق قبل أن يرى أي طالب الاختبار.",
      ...
      m3t:"المعلم يراجع",            m3b:"يمكنه الموافقة أو الرفض أو تعديل أي سؤال قبل وصوله للطلاب.",
      ...
      q3t:"المعلم يراجع",             q3b:"نفس عملية الموافقة — لا يرى أي طالب شيئاً حتى يعطي المعلم الضوء الأخضر.",
      pers_t:"اختبارات مخصصة تتكيف مع كل طالب",
      pers_b:"يرى النظام أداء كل طالب السابق ويضبط مستوى الصعوبة تلقائياً — الطلاب المتعثرون يحصلون على أسئلة أسهل، والمتفوقون على أصعب.",
      lock_t:"لا شيء يُنشر دون موافقة المعلم",
      lock_b:"كل اختبار يبدأ كمسودة. لا يمكن للطلاب رؤيته حتى يوافق المعلم صراحةً عليه.",
...
const CorrectionContent = ({ lang }) => {
  ...
  const s = {
    ...
    ar: {
      ...
      mc1t:"الطالب يُجري الاختبار",    mc1b:"يجيب الطالب على كل سؤال باختيار خيار من الأربعة.",
      mc2t:"التصحيح فوري",              mc2b:"في اللحظة التي يُرسل فيها، تظهر النتيجة. لا انتظار.",
      mc3t:"تفصيل كامل يُعرض",         mc3b:"يرى الطالب ما أصابه وما أخطأ فيه، مع شرح لكل إجابة.",
      ...
      statuses:[["مُرسَل","الطالب سلّمه"],["صُحِّح بالذكاء","الذكاء الاصطناعي قيّمه"],["مراجعة المعلم","المعلم يفحصه"],["مُنتهٍ","الدرجة رسمية"]],
    },
  }[lang];
...
const NotebookContent = ({ lang }) => {
  ...
  const s = {
    ...
    ar: {
      intro:"كل طالب لديه دفتر رقمي يعمل كسجل يومي. كل يوم يحصل على صفحته. يضع المعلمون التمارين، يكتب الطلاب الإجابات، ويصحح المعلمون ويعلقون — كل شيء في مكان واحد.",
      st:"تجربة الطالب", te:"تجربة المعلم",
      s1t:"يفتح صفحة اليوم",  s1b:"صفحة اليوم موجودة تلقائياً عند الفتح.",
      s2t:"يرى التمارين",     s2b:"تظهر التمارين في أعلى الصفحة ويكتب الطالب إجاباته.",
      s3t:"يحصل على ملاحظات", s3b:"بعد التصحيح يرى الطالب علاماته وتعليقات المعلم.",
      t1t:"يبحث عن طالب",   t1b:"قائمة الطلاب قابلة للبحث في تبويب الدفاتر.",
      t2t:"يحدد التمارين",   t2b:"يكتب المعلم التمارين مباشرة في صفحة الطالب.",
      t3t:"يصحح ويعلق",     t3b:"يحدد علامة لكل إجابة ويكتب تعليقاً. الطالب يراه فوراً.",
      loop_t:"الحلقة الكاملة", loop_b:"المعلم يكتب التمارين ← الطالب يجيب ← المعلم يصحح ← الطالب يقرأ الملاحظات — في نفس الصفحة.",
    },
  }[lang];
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
...
const CnpContent = ({ lang }) => {
  ...
  const s = {
    ...
    ar: {
      ...
      acc_title:"صلاحيات الوصول", acc_b:"هيئة المناهج ترفع وترى ملفاتها. المشرفون يديرون ويعتمدون. المعلمون والمستشارون يرون المعتمد فقط. الطلاب وأولياء الأمور لا يملكون وصولاً.",
    },
  }[lang];
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
...
const AdvisorContent = ({ lang }) => {
  const s = {
    ...
    ar: {
      ...
      hi_b:"المعلم يعتمد للطلاب. المستشار يراجع للجودة. رأي المستشار لا يحجب ما يراه الطلاب أبداً.",
      jour_t:"الرحلة الكاملة لاختبار",
      journey:[
        { label:"الذكاء الاصطناعي يُنشئ",  sub:"الأسئلة تُولَّد من محتوى الدرس",          color:T.purple, Icon:Brain         },
        { label:"المعلم يراجع ويعتمد",     sub:"يعدّل إذا لزم ثم ينشر للطلاب",             color:T.blue,   Icon:GraduationCap },
        { label:"الطلاب يجرون الاختبار",  sub:"يصبح مرئياً ويكمله الطلاب",               color:T.green,  Icon:User          },
...
const InspectionContent = ({ lang }) => {
  ...
  const s = {
    ...
    ar: {
      intro:"تدعم المنصة نظامَي تفتيش متوازيَين على مستويين مختلفين — أحدهما للمناطق والآخر للمدارس والمعلمين. كلاهما مستقل ولا يشمل ذكاءً اصطناعياً.",
      reg:"التفتيش الإقليمي", sch:"التفتيش على مستوى المدرسة",
      r1t:"يُعيَّن المفتش لمنطقة",     r1b:"مسؤول عن منطقة جغرافية تشمل مدارس متعددة.",
      r2t:"يجدول الزيارات وينفذها",     r2b:"تشمل زيارات دورية، متابعة شكاوى، مراجعات سنوية، وتفتيشاً مفاجئاً.",
      r3t:"يكتب تقريراً",              r3b:"بعد كل زيارة يُرفع تقرير رسمي للمفتش العام.",
      r4t:"المفتش العام يعتمد أو يُعيد", r4b:"يمكن الاعتماد أو الرفض أو طلب مراجعات. الملخصات الشهرية تمر بنفس المسار.",
      s1t:"المديرية تجدول التفتيش",    s1b:"تُنشئ السلطة التعليمية تفتيشاً لمعلم محدد في مدرسته.",
      s2t:"قد يتولاه مستشار",           s2b:"يمكن تعيين مستشار لإجراء التفتيش — مفيد للتقييمات المتخصصة.",
      s3t:"اكتمال التفتيش",            s3b:"مراحل: مجدول ← قيد التنفيذ ← مكتمل ← مُقدَّم، مع تقرير كامل.",
...
const PROCEDURES = [
  ...
  { id:"notebook",   num:"06", icon:NotebookPen,   hasAi:false, accent:T.pink,
    title:{ en:"Student Notebooks",               ar:"دفاتر الطلاب"              },
    tagline:{ en:"A daily digital notebook connecting teachers and students.", ar:"دفتر رقمي يومي يربط المعلمين بالطلاب." },
  ...
  { id:"inspection", num:"09", icon:Search,         hasAi:false, accent:T.slate,
    title:{ en:"Inspections & Oversight",          ar:"التفتيش والرقابة"          },
    tagline:{ en:"Regional and school-level inspections, all tracked digitally.", ar:"التفتيش الإقليمي وعلى مستوى المدرسة — كل شيء مُتتبَّع رقمياً." },
